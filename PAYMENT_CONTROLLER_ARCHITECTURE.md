...
| Gateway | Legacy v1 (`get_payment_url`) | PaymentController v2 | Notes |
|---------|------------------------------|---------------------|-------|
| Stripe | ✅ Supported | ✅ Supported | v2 uses PaymentIntent API |
| PayPal | ✅ Supported | ❌ Not yet | |
| Razorpay | ✅ Supported | ❌ Not yet | |
| Braintree | ✅ Supported | ❌ Not yet | |
| GoCardless | ✅ Supported | ❌ Not yet | |
| Mollie | ✅ Supported | ❌ Not yet | |
| Paytm | ✅ Supported | ❌ Not yet | |
| M-Pesa | ✅ Supported | ❌ Not yet | |
| Paymob | ✅ Supported | ❌ Not yet | |

### Version Requirements

- **Python**: 3.10+
- **Frappe Framework**: 15.x (develop branch)
- **stripe-python**: ~=10.12.0 (10.x series)
- **Stripe API**: Pinned to `2025-12-15.clover`

### Migration Checklist for v1 Gateway Users

The PaymentController architecture is **opt-in** and does not break existing integrations:

1. **No immediate action required** - Existing `get_payment_url()` flows continue to work
2. **Optional adoption** - To use PaymentController:
   - Ensure your gateway settings DocType implements the PaymentController interface
   - Create a Payment Button pointing to your gateway settings
   - Use `PaymentController.initiate()` instead of `get_payment_url()`

### Migration Path for Gateway Implementors

To adopt PaymentController for your gateway:

1. **Implement the interface** in your gateway settings DocType:
   ```python
   from payments.controllers import PaymentController

   class MyGatewaySettings(Document, PaymentController):
       def validate_tx_data(self, tx_data: TxData) -> None:
           # Validate currency, amount, etc.
           pass

       def _initiate_charge(self) -> Initiated:
           # Create payment on remote gateway
           # Return Initiated with correlation_id and payload
           pass

       def _validate_response(self) -> None:
           # Verify webhook signature or client response
           pass

       def _process_response_for_charge(self) -> Processed | None:
           # Map gateway status to Success/Failed/Processing
           pass
   ```

2. **Create frontend templates** for the Payment Button:
   - `gateway_css`: Styles for the checkout widget
   - `gateway_js`: JavaScript for initializing the gateway SDK
   - `gateway_wrapper`: HTML wrapper for the payment form

3. **Create a Payment Button** via Desk:
   - Link to your gateway settings DocType
   - Add your frontend templates
   - Enable the button

4. **Handle webhooks** by implementing:
   ```python
   @frappe.whitelist(allow_guest=True)
   def my_gateway_webhook():
       # Verify signature, extract PSL name from metadata
       PaymentController.process_response(psl_name, payload, hash)
   ```

### Webhook Events (Stripe)

The Stripe integration handles these webhook events:

| Event | Handled | Notes |
|-------|---------|-------|
| `payment_intent.succeeded` | ✅ Yes | Updates PSL to Success |
| `payment_intent.payment_failed` | ✅ Yes | Updates PSL to Failed |
| `payment_intent.canceled` | ✅ Yes | Updates PSL to Cancelled |
| `payment_intent.requires_action` | ❌ No | 3D Secure handled client-side by Stripe.js |
| `charge.refunded` | ❌ No | Refunds managed via Frappe document workflows |
| `charge.dispute.created` | ❌ No | Disputes require manual review in Stripe Dashboard |
| `payment_intent.amount_capturable` | ❌ No | Separate auth/capture not implemented |

Unhandled events return `{"status": "ignored"}` with HTTP 200 to prevent Stripe retries.
...
This creates:
- `Payment Session Log` DocType with indexes on `status` and `correlation_id`
- `Payment Button` DocType for gateway widget configuration
